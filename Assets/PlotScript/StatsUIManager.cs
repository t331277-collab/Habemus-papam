...
public class StatsUIManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI[] plotNames = new TextMeshProUGUI[3];
    public TextMeshProUGUI[] plotDescs = new TextMeshProUGUI[3];
    public TextMeshProUGUI[] plotEffect = new TextMeshProUGUI[3];
    ...
    /* 함수 이름 : OnClickPlot
     * 함수 기능 : Plot 버튼이 클릭 됐을 때 호출하여 게임의 시간을 멈추고 3개의 증강을 띄운다
     * 함수 파라미터 : 없음
     * 반환값 : 없음
     */
     ...
     * 반환값 : 없음
     */
    public void OnClickSelectPlot(int index)
    {
        bool canSelect;

        canSelect = gameManager.OnSelectPlot(index);
        ...
        if (canSelect)
        {
            plotButtons[index].interactable = false;
            actionText.text = "Conducting Operations...";
        }
        else
        {
            actionText.text = "Not enough piety to use this plot";
        }
    ...
    /* 함수 이름 : ResetPlotUI
     * 함수 기능 : 증강 선택 화면의 버튼을 초기상태로 리셋 (이름과 설명 빈 문자열, 버튼 비활성화)
     * 함수 파라미터 : 없음
     * 반환값 : 없음
     */
    ...
    /* 함수 이름 : SetPlotUI
     * 함수 기능 : 증강 선택 화면에서 선택 가능한 증강의 정보를 UI로 전달 및 버튼 활성화
     * 함수 파라미터 : List<Plot> availPlotList, 이번 턴에 선택 가능한 세 가지 증강을 인자로 받음
     * 반환값 : 없음
     */
    public void SetPlotUI(List<Augment> availPlotList)
    {
        for (int i = 0; i < availPlotList.Count; i++)
        {
            plotNames[i].text = availPlotList[i].plotName;     // 공작 이름 전달
            plotDescs[i].text = availPlotList[i].plotDesc;     // 공작 설명 전달
            plotEffect[i].text = availPlotList[i].plotEffect + $"\n 비용 : {availPlotList[i].pietyCost}";     // 공작 효과 및 비용 전달
            plotButtons[i].interactable = true;
        }
    }
     ...
     * 함수 파라미터 : List<Character> characters, 캐릭터들의 스탯을 저장한 리스트를 인자로 받음
     * 반환값 : 없음
     */
