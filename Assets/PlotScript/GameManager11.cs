 ...
 *              plotManager         PlotManager 스크립트를 참조하는 변수
 ...
 *             OnSelectPlot         UI에서 세개의 증강 중 하나를 선택했을 때 선택된 증강의 index를 받아 선택된 증강을 저장하는 SelectPlot 함수 실행
 */
public class GameManager1 : MonoBehaviour
{
    ...
    public PlotManager plotManager;   // PlotManager 스크립트를 참조하는 변수
    ...
    void Awake()
    {
        ...
        plotManager = GetComponent<PlotManager>();    // PlotManager 컴포넌트를 받아 저장
    ...
    void Update()
    {
        ...
        statsUIManager.SetStatsInfo(statsManager.cardinals);
    ...
    void NextTurn()
    {
        plotManager.UpdatePlotBuffer(statsManager.cardinals[0]); // 새로운 턴에서 사용 가능한 증강 업데이트
        plotManager.GetTotalWeight();    // 이번 턴에 사용 가능한 증강들의 총 가중치 합 구하기
        plotManager.GetThreePlot();   // 가중치 뽑기를 통해 선택 창에 띄울 세가지 증강 저장

        statsUIManager.ResetPlotUI();    // 증강 선택 UI 초기화
        statsUIManager.SetPlotUI(plotManager.availPlotList);   // 세가지 증강을 UI 정보에 전달
    ...
    // UI에서 세개의 증강 중 하나를 선택했을 때 선택된 증강의 index를 받아 공작의 사용 가능 여부를 bool로 반환하는 함수
    public bool OnSelectPlot(int index)
    {
        return plotManager.SelectPlot(index, statsManager.cardinals);
    }
