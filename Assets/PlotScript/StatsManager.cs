...
public class Character
{
    ...
    [SerializeField] public int influence;
    [SerializeField] public int piety;

    // 입력 받은 값으로 능력치 초기화
    public Character(int piety, int influence, float hp)
    {
        this.piety = piety;
    ...
    {
        piety = Mathf.Clamp(piety + amount, 0, 100);
    }
...
public class StatsManager : MonoBehaviour
{
    ...
    // 연설 관련 변수
    public int influenceSpeechChange; // 연설로 인한 정치력 변화량
    public int influenceSpeechRate;   // 연설로 인한 정치력 변화 확률
    ...
    {
        // 0번 인덱스 플레이서 능력치
        cardinals.Add(new Character(Random.Range(25, 35), Random.Range(25, 35), 100f));
        // 1~3번 인덱스 상대 후보 능력치
        cardinals.Add(new Character(Random.Range(25, 35), Random.Range(25, 35), 100f));
        cardinals.Add(new Character(Random.Range(25, 35), Random.Range(25, 35), 100f));
        cardinals.Add(new Character(Random.Range(25, 35), Random.Range(25, 35), 100f));
        ...
        // 연설 관련 변수 초기화
        influenceSpeechChange = 10;   // 연설로 인한 정치력 변화량
        influenceSpeechRate = 70;     // 정치력 변화 확률
    }
    ...
    public void Pray()
    {
        cardinals[0].IncreasePiety(pietyPrayChange);     // 경건함을 변화

        if (Random.Range(0f, 100f) <= hpPrayRate)   // 체력이 회복될 확률 적용
        {
            cardinals[0].IncreaseHp(hpPrayChange);       // 확률에 들어오면 체력 회복
        }
    ...
    public void Speech()
    {
        if (Random.Range(0f, 100f) <= influenceSpeechRate)    // 정치력이 변화할 확률 적용
        {
            cardinals[0].IncreaseInfluence(influenceSpeechChange);       // 확률에 들어오면 정치력 변화
        }
    }
